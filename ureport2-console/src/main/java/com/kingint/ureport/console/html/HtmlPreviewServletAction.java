 ...
 * the License.
 ******************************************************************************/
package com.kingint.ureport.console.html;
...
import com.kingint.ureport.build.ReportBuilder;
...
import com.kingint.ureport.console.MobileUtils;
...
import com.kingint.ureport.definition.searchform.FormPosition;
import com.kingint.ureport.exception.ReportComputeException;
import com.kingint.ureport.export.ExportManager;
import com.kingint.ureport.export.FullPageData;
import com.kingint.ureport.export.PageBuilder;
import com.kingint.ureport.export.ReportRender;
import com.kingint.ureport.export.SinglePageData;
import com.kingint.ureport.export.html.HtmlProducer;
import com.kingint.ureport.export.html.HtmlReport;
import com.kingint.ureport.export.html.SearchFormData;
...
public class HtmlPreviewServletAction extends RenderPageServletAction {
	...
	public void loadPrintPages(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if(report.getHeader()!=null){
			try{
				com.kingint.ureport.build.paging.HeaderFooter hf=report.getHeader().buildHeaderFooter(1,report.getContext());
		...
		if(report.getFooter()!=null){
			try{
				com.kingint.ureport.build.paging.HeaderFooter hf=report.getFooter().buildHeaderFooter(1,report.getContext());
	...
	private void applyPaperToHtmlReport(Paper paper, Report report, HtmlReport htmlReport) {
		...
		if (report.getHeader() != null && report.getPages().size() > 0) {
			com.kingint.ureport.build.paging.HeaderFooter hf = report.getHeader()
		...
		if (report.getFooter() != null && report.getPages().size() > 0) {
			com.kingint.ureport.build.paging.HeaderFooter hf = report.getFooter()
				.buildHeaderFooter(1, report.getContext());
			htmlReport.setFooterHtml(buildHfHtml(hf, false));
		}
	}

	private String buildHfHtml(com.kingint.ureport.build.paging.HeaderFooter hf, boolean isHeader) {
		StringBuilder sb = new StringBuilder();
	...
	private String buildAllPagesHtml(Report report) {
		FullPageData pageData = PageBuilder.buildFullPageData(report);
		Context context = report.getContext();
		com.kingint.ureport.definition.Paper paper = report.getPaper();
	...
	private String buildPageSheet(String pageHtml, Report report, int pageNum, int mt, int mb, int ml, int mr) {
		...
		if (hasHeader) {
			...
			try {
				com.kingint.ureport.build.paging.HeaderFooter hf =
