...
public class AiServletAction extends RenderPageServletAction {
	...
	public void generate(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		try {
			...
			if (!config.isEnabled()) {
				Map<String, Object> error = new HashMap<String, Object>();
				error.put("error", "AI功能未启用。请在config.properties中设置 ureport.ai.enabled=true");
				writeObjectToJson(resp, error);
				return;
			}
	...
	private void saveReport(String file, ReportDefinition reportDef) throws Exception {
		...
		Collection<com.kingint.ureport.provider.report.ReportProvider> providers =
				applicationContext.getBeansOfType(
						com.kingint.ureport.provider.report.ReportProvider.class).values();

		for (com.kingint.ureport.provider.report.ReportProvider provider : providers) {
