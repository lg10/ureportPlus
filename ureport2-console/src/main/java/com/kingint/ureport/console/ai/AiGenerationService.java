...
public class AiGenerationService {
	...
	private String buildValidatorPrompt() {
		...
		sb.append("你是一个 UReport2 报表表达式校验专家。\n");
		sb.append("你的任务是检查 Generator 生成的单元格修改方案是否正确。\n\n");
		sb.append("校验规则：\n");
		sb.append("1. 检查表达式语法是否符合 UReport2 规范\n");
	...
	public List<Map<String, Object>> applyModifications(List<CellModification> modifications) {
		...
		for (CellModification mod : modifications) {
			...
			if (mod.expand != null) {
				try { cell.setExpand(com.kingint.ureport.definition.Expand.valueOf(mod.expand)); } catch (IllegalArgumentException ignored) {}
			}
