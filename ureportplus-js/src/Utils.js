...
export function showLoading(){
    const url=window._server+'/res/ureportplus-asserts/icons/loading.gif';
    const h=$(window).height()/2,w=$(window).width()/2;
    const cover=$(`<div class="ureportplus-loading-cover" style="position: absolute;left: 0px;top: 0px;width:${w*2}px;height:${h*2}px;z-index: 1199;background:rgba(222,222,222,.5)"></div>`);
    $(document.body).append(cover);
    const loading=$(`<div class="ureportplus-loading" style="text-align: center;position: absolute;z-index: 1120;left: ${w-35}px;top: ${h-35}px;"><img src="${url}">
    <div style="margin-top: 5px">打印数据加载中...</div></div>`);
    $(document.body).append(loading);
};

export function hideLoading(){
    $('.ureportplus-loading-cover').remove();
    $('.ureportplus-loading').remove();
};
...
export function tableToXml(context){
    ...
    let xml=`<?xml version="1.0" encoding="UTF-8"?><ureportplus>`;
