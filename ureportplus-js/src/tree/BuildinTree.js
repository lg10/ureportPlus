...
export default class BuildinTree extends BaseTree{
    ...
    _buildDatasource(){
        ...
        const rootSpan=$(`
            <span id="${this.id}">
                <i class='ureportplus ureportplus-minus' style='margin-right:2px'></i>
                <i class="ureportplus ureportplus-shareconnection"></i> <a href='###' class="ds_name">${this.name}</a>
            </span>`);
    ...
    addDataset(dataset){
        ...
        const span=$(`<span id="${spanId}"><i class='ureportplus ureportplus-minus' style='margin-right:2px'></i> <i class="ureportplus ureportplus-sqlds"></i> <a href='###' class="dataset_name">${dataset.name}</a></span>`);
        li.append(span);
    ...
    addField(dataset,fields,field,ul){
        ...
        const li=$(`<li></li>`);
        const spanId=uuid.v1();
        const span=$(`<span id="${spanId}" title="${window.i18n.tree.doubleClick}"><i class="ureportplus ureportplus-property"></i> <a href='###'>${field.name}</a></span>`);
        li.append(span);
    ...
    attachEvent(span,li){
        span.click(function (e) {
            ...
            if ($liChildren.is(":visible")) {
                ...
                span.children('i:first').addClass('ureportplus-plus').removeClass('ureportplus-minus');
            } else {
                $liChildren.show('fast');
                span.children('i:first').addClass('ureportplus-minus').removeClass('ureportplus-plus');
            }
            e.stopPropagation();
        });
    }
