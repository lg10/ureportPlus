...
import com.kingint.ureportplus.console.auth.ReportAuthCheck;
import com.kingint.ureportplus.console.auth.UnauthorizedException;
...
public abstract class BaseServletAction implements ServletAction, ApplicationContextAware {
	...
	protected String retriveMethod(HttpServletRequest req) throws ServletException{
		String path=req.getContextPath()+UReportPlusServlet.URL;
	...
	protected String buildDownloadFileName(String reportFileName,String fileName,String extName){
		if(StringUtils.isNotBlank(fileName)){
			...
			pos=reportFileName.toLowerCase().indexOf(".ureportplus.xml");
			if(pos>0){
				reportFileName=reportFileName.substring(0,pos);
			}
			return "ureportplus-"+reportFileName+extName;
		}
	}
}
